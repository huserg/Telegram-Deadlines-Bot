...
bot.start((ctx) => {
    // if admins is empty, set ctx.from.user_id as admin with c.commands.admin else do nothing
});

bot.command('admin', (ctx) => c.commands.admin(ctx));
...
/*bot.on('inline_query', ctx => {
    ...
    if(query.startsWith("add")){
        ...
        if(message_length > 0) {
            let full_message = 'Does it correspond to "add Subject;Date(DD-MM-AAAA);Theme" ?';

            return ctx.answerInlineQuery([{
                type: 'article',
                id: ctx.update.inline_query.id,
                title: 'You have inserted: ' + new_input,
                description: 'Does it correspond to "add Subject;Date(DD-MM-AAAA);Theme" ?',
